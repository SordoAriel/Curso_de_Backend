...
class ProductsManager extends BasicManager {
  ...
  async getWithAdvancedSearch(obj){
    ...
    const prevLink = advancedSearch.hasPrevPage ? `/products?page=${advancedSearch.prevPage}&limit=${limit}` : null
    const nextLink = advancedSearch.hasNextPage ? `/products?page=${advancedSearch.nextPage}&limit=${limit}` : null
    const response = {
